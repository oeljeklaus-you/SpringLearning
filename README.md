...
## Spring消息
 异步消息是一个程序向另一个程序间接发送消息的一种方式,这种方式无需等待对方的响应。
### 异步消息简介
#### 发送消息
 异步消息的两个主要概念:消息代理和目的地
 
 有两种通用的目的地:队列和主题。每种类型都与特定的消息模型相关联，分别是点对点模式和发布订阅模式。
 
 点对点模式:
 ![点对点模式](img/点对点模式.png)
 发布-订阅消息模式:
 ![发布-订阅模式](img/发布-订阅模式.png)
#### 评估异步消息的优点
 采用同步通信机制访问远程服务的客户端几个限制,主要是:
 ...
 4.确保独立
### 使用JMS发送消息
 JMS是一个Java标准,定义了使用消息代理的通用API。Spring提供了基于模板的抽象为JMS功能提供支持.这个模板是JMTemplate。
#### 在Spring中搭建信息代理
 1.在本机中使用activemq
 
 2.在工程中添加了pom文件
 ![activemq的pom](img/activemq的pom.png)
 
 3.创建连接工厂
 ![配置链接工厂](img/配置链接工厂.png)
 4.声明ActiveMQ消息目的地
 
 目的地可以是一个队列或者主题。不论是队列还是主题,必须使用特定的消息代理实现类在Spring中配置目的地bean。
 ![配置的实体或者主题](img/配置的实体或者主题.png)
 类似于连接工程可以使用ActiveMQ命名空间来进行配置
 ![ActiveMQ命名空间来配置](img/ActiveMQ命名空间来配置.png) 
#### 使用Spring的JMS模板
 使用JMS模板，可以防止JMS抛出的受查时异常转化为非受查时异常,主要的异常有:
 ![JMS](img/JMS.png)
 在XML配置bean
 ![在xml中配置jmsTemplate](img/在xml中配置jmsTemplate.png)
 发送消息
 ![配置发送消息](img/配置发送消息.png)
 设置默认目的地址
 ![设置默认的目的地址](img/设置默认的目的地址.png)
 发送时对消息进行转化:
 ![发送时对消息进行转化](img/发送时对消息进行转化.png)
 MessageConverter是Spring的接口,Spring提供了多个消息转化器:
 ![消息转化器](img/消息转化器.png)
 如果我们需要使用JSON消息转化器可以这样配置:
 ![xml中配置配置消息转化器](img/xml中配置配置消息转化器.png)
 接收消息
 ![接收消息](img/接收消息.png)
 接收消息的方法是同步的，意味着等待消息的来临并接受
#### 创建消息驱动的POJO
 EJB2.0引入了消息驱动bean,MDB。换句话说,MDB将JMS目的地中的消息作为事件,并对这些事件进行响应。
 
 为了进一步简化MDB， 我们只需要在POJO实现更加通用的javax.jms.MessageListener接口,并使用
 
 @MessageDriver注解标注MDB.
